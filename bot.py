...
bot_token = 'a86730aab5c59953c424abb4396d32d5'
api_id = '14681595'
    ...
    try:
        ...
        if req.status_code == 200:
            ...
            if file_size > chunk_size:
                ...
                mult_file.close()
                index = 1
                await msg.edit('Preparando para subir a la nube ☁...')
                ...
                if loged == True:
                    ...
                    if 'url' in resp:
                        ...
                        msgurls += '\n' + data[0] + '\n'
                os.unlink(file_name)
            await msg.edit('⏩URL De los archivos comprimidos! '+str(msgurls) + '\n' + str(file_name))
            txtfile = open(str(file_name)+'.txt','w')
            txtfile.write(msgurls)
            txtfile.close()
            txtfile = open(str(file_name)+'.txt','rb')
            await bot.send_file(ev.message.chat,txtfile)
            txtfile.close()
            os.unlink(str(file_name)+'.txt')
            await bot.send_message(ev.chat_id, '📡Archivos subidos a la cuenta... \n Usuario-> '+str(conf.moodleUser)+'\n Contraseña->'+str(conf.moodlepassword))
    except Exception as e:
...
async def process_message(text,bot,ev,msg):
    if '/dtel' in text:
        ...
        await msg.edit('🖇El tamaño de los zip al subir a la nube☁ a Cambiado✂️')
    elif '/sct ' in text:
    ...
    try:
        ...
        if file_size > chunk_size:
            ...
            for f in multiFile.files:
               ...
               # await bot.send_file(ev.message.chat,f)
                progres  = await text_progres(index,len(multiFile.files))
                await msg.edit('subiendo a la nube...')
                moodle = MoodleClient(conf.moodleUser,conf.moodlepassword)
                loged = moodle.login()
                if loged == True:
                    resp = moodle.upload_file(f)
            ...
            if loged == True:
                ...
                if 'url' in resp:
                    ...
                    msgurls += '\n' + data[0] + '\n'
                os.unlink(file)
        await msg.edit('⏩URL Del archivo no comprimido! '+str(msgurls) + '\n' + str(file))
        txtfile = open(str(file)+'.txt','w')
        txtfile.write(msgurls)
        txtfile.close()
        txtfile = open(str(file)+'.txt','rb')
        await bot.send_file(ev.message.chat,txtfile)
        txtfile.close()
        os.unlink(str(file)+'.txt')
        await bot.send_message(ev.chat_id, '📡Archivo Subido a la cuenta.... \n usuario=> '+str(conf.moodleUser)+'\n contraseña=>'+str(conf.moodlepassword))
    except Exception as e:
